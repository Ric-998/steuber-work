...
function ChatTab({ currentUserId }: { currentUserName: string; currentUserId: string }) {
  ...
  const loadData = async () => {
    setLoading(true)
    // Load all active users except self
    ...
    // Load latest message per conversation partner
  ...
  useEffect(() => {
    loadData()
    // Realtime: reload on any message change
  ...
  if (openUserId) {
    ...
    return (
      <ChatConversation
        ...
        onBack={() => { setOpenUserId(null); loadData() }}
      />
    )
  }
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 40 }}>
          ...
          <div style={{ fontSize: 14, color: 'var(--txt-muted)' }}>
            {search ? 'Keine Ergebnisse' : 'Noch keine Nachrichten'}
          </div>
          {!search && (
            <div style={{ fontSize: 12, color: 'var(--txt-muted)', marginTop: 4 }}>Tippe auf + um eine Unterhaltung zu starten</div>
          )}
        </div>
      ) : (
        <>
          {hasAnyMsg && <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--txt-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8, marginTop: 6 }}>Chats</div>}
          {filtered.map(user => {
            const lastMsg = lastMessages[user.id]
            const unread = unreadCounts[user.id] || 0
            const roleName = (user.role as any)?.name === 'admin' ? 'Chef · Steuber GmbH' : (user.role as any)?.name || 'Mitarbeiter'
            return (
              <button key={user.id} onClick={() => setOpenUserId(user.id)}
                ...
                <div style={{ position: 'relative', flexShrink: 0 }}>
                  <div style={{ width: 46, height: 46, borderRadius: '50%', background: 'var(--pri-xl)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 800, color: 'var(--pri)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 13, color: 'var(--txt-muted)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '80%' }}>
                      {lastMsg
                        ? (lastMsg.sender_id === currentUserId ? `Du: ${lastMsg.text}` : lastMsg.text)
                        : roleName}
                    </span>
                    {unread > 0 && (
      ...
      {showNewChat && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200 }}>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'var(--bg)', borderRadius: '20px 20px 0 0', padding: '20px 16px 40px', maxHeight: '75vh', overflowY: 'auto' }}>
            ...
            {users.map(user => {
              const roleName = (user.role as any)?.name === 'admin' ? 'Chef · Steuber GmbH' : 'Mitarbeiter/in'
              return (
                <button key={user.id} onClick={() => { setShowNewChat(false); setOpenUserId(user.id) }}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--txt-muted)' }}>{roleName}</div>
                  </div>
                </button>
              )
            })}
...
function ChatConversation({ contact, currentUserId, onBack }: {
  contact: DbUser
  ...
  useEffect(() => {
    loadMessages()
    const channel = supabase.channel(`chat-conv-${contact.id}`)
      .on('postgres_changes', {
  ...
  const roleName = (contact.role as any)?.name === 'admin' ? 'Chef · Steuber GmbH' : 'Mitarbeiter/in'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 140px)', background: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0 10px', background: 'var(--surf-card)', borderBottom: '1px solid var(--outline)', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--txt-muted)', fontWeight: 600 }}>{roleName}</div>
        </div>
      </div>
